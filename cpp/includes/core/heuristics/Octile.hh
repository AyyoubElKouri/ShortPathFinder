...
/**
 * @brief Octile distance heuristic implementation.
 * 
 * This class computes the Octile distance between two nodes in a graph.
 */
class Octile : public IHeuristic {
  ...
  public:
    /**
     * @brief Constructor for the Octile heuristic.
     * 
     * @param graph Reference to the graph on which the heuristic will operate.
     */
    explicit Octile( std::shared_ptr<const IGraph> graph ) : graph_(graph) {}

    /**
     * @brief Compute the Octile distance between two nodes.
     * 
     * @param from The NodeId of the starting node.
     * @param to The NodeId of the target node.
     * 
     * @return The Octile distance as a Cost type.
     * 
